<x-app-layout>
    <x-slot name="header">Nuovo profilo</x-slot>
    <x-slot name="actions">
    ...
    <div class="bg-white shadow-sm rounded-xl">
        <form method="POST" action="{{ route('admin.profili.store') }}" class="p-6 space-y-5"
              x-data="{
                limitaIstituto: {{ old('limita_istituto') ? 'true' : 'false' }},
                limitaSeg: {{ old('limita_segnalazioni', 0) }}
              }">
            @csrf
                            ...
                            <optgroup label="{{ $tipo }}">
                                @foreach($items as $org)
                                    <option value="{{ $org->id_istituto }}"
                                        {{ old('id_istituto') == $org->id_istituto ? 'selected' : '' }}>
                                        {{ $org->descrizione }}
                                    </option>
                                @endforeach
                            </optgroup>
                ...
                <select id="id_tipologia_segnalazione" name="id_tipologia_segnalazione"
                    ...
                    @foreach($tipologie as $tip)
                        <option value="{{ $tip->id_tipologia_segnalazione }}"
                            {{ old('id_tipologia_segnalazione') == $tip->id_tipologia_segnalazione ? 'selected' : '' }}>
                            {{ $tip->descrizione }}
                        </option>
                    @endforeach
                </select>
