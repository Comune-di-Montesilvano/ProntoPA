<x-app-layout>
    <x-slot name="header">Modifica utente — {{ $utente->username }}</x-slot>
    <x-slot name="actions">
    ...
    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
        <form method="POST" action="{{ route('admin.utenti.update', $utente->id) }}" class="p-6 space-y-5 max-w-2xl">
            @csrf @method('PATCH')

            @php $ruoloCorrente = old('ruolo', $utente->getRoleNames()->first() ?? 'segnalatore'); @endphp

            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    <x-input-label for="name" value="Nome completo *" />
                    <x-text-input id="name" name="name" type="text" class="mt-1 block w-full"
                        :value="old('name', $utente->name)" required />
                    <x-input-error :messages="$errors->get('name')" class="mt-1" />
                </div>
                <div>
                    <x-input-label for="username" value="Username *" />
                    <x-text-input id="username" name="username" type="text" class="mt-1 block w-full"
                        :value="old('username', $utente->username)" required autocomplete="off" />
            ...
            <div>
                ...
                <select id="ruolo" name="ruolo"
                    ...
                    <option value="admin"       {{ $ruoloCorrente === 'admin'       ? 'selected' : '' }}>Admin</option>
                    <option value="gestore"     {{ $ruoloCorrente === 'gestore'     ? 'selected' : '' }}>Gestore</option>
                    <option value="segnalatore" {{ $ruoloCorrente === 'segnalatore' ? 'selected' : '' }}>Segnalatore</option>
                    <option value="impresa"     {{ $ruoloCorrente === 'impresa'     ? 'selected' : '' }}>Impresa</option>
                </select>
                <x-input-error :messages="$errors->get('ruolo')" class="mt-1" />
            </div>

            {{-- Supervisore --}}
            <div x-data="{ show: '{{ $ruoloCorrente }}' === 'gestore' }"
                ...
                <label class="flex items-center gap-2 cursor-pointer">
                    <input type="checkbox" name="supervisore" value="1"
                           ...
                           {{ old('supervisore', $utente->supervisore_segnalazioni) ? 'checked' : '' }}>
            ...
            <div>
                <label class="flex items-center gap-2 cursor-pointer">
                    <input type="checkbox" name="attivo" value="1"
                           class="rounded border-gray-300 text-blue-600 shadow-sm"
                           {{ old('attivo', $utente->attivo ?? true) ? 'checked' : '' }}>
                ...
                <select id="id_profilo" name="id_profilo"
                    ...
                    @foreach($profili as $p)
                        <option value="{{ $p->id_profilo }}"
                            {{ old('id_profilo', $utente->id_profilo) == $p->id_profilo ? 'selected' : '' }}>
                            {{ $p->descrizione }}
                ...
                <select id="id_provenienza" name="id_provenienza"
                    ...
                    @foreach($provenienze as $p)
                        <option value="{{ $p->id_provenienza }}"
                            {{ old('id_provenienza', $utente->id_provenienza) == $p->id_provenienza ? 'selected' : '' }}>
                            {{ $p->descrizione }}
                        </option>
                    @endforeach
                </select>
            ...
            {{-- Impresa --}}
            <div x-data="{ show: '{{ $ruoloCorrente }}' === 'impresa' }"
                ...
                <select id="id_impresa" name="id_impresa"
                    ...
                    @foreach($imprese as $i)
                        <option value="{{ $i->id_impresa }}"
                            {{ old('id_impresa', $utente->id_impresa) == $i->id_impresa ? 'selected' : '' }}>
                            {{ $i->ragione_sociale }}
                        </option>
                    @endforeach
                </select>
