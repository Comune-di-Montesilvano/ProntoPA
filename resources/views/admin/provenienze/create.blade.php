<x-app-layout>
    <x-slot name="header">Nuova provenienza</x-slot>
    <x-slot name="actions">
    ...
    <div class="bg-white shadow-sm rounded-xl">
        <form method="POST" action="{{ route('admin.provenienze.store') }}" class="p-6 space-y-5">
            @csrf

            <div>
                <x-input-label for="descrizione" value="Descrizione *" />
                <x-text-input id="descrizione" name="descrizione" type="text"
                              class="mt-1 block w-full" :value="old('descrizione')"
                              required maxlength="50" placeholder="Es. Scuola, URP, Portale web…" />
