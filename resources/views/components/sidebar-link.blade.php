...
@php
$base     = 'flex items-center gap-2.5 px-3 py-2 rounded-lg text-sm font-medium transition-colors';
$activeCs = 'bg-gray-700 text-white';
$inactiveCs = 'text-gray-300 hover:bg-gray-800 hover:text-white';
$classes  = $base . ' ' . ($active ? $activeCs : $inactiveCs);
@endphp

<a href="{{ $href }}"
   {{ $attributes->merge(['class' => $classes]) }}>
    {{-- Heroicon mini inline --}}
    @if($icon === 'clipboard')
